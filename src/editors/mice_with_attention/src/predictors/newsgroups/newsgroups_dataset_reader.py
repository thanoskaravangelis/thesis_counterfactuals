...
class NewsgroupsDatasetReader(DatasetReader):
    ...
    def get_data_indices(self, subset, inputs_path):
        np.random.seed(self.random_seed)
        if subset in ['train', 'test']:
            if inputs_path is None:
                newsgroups_data = fetch_20newsgroups(
                        data_home="/users/pa21/ptzouv/tkaravangelis/scikit_learn_data",
    ...
    def get_inputs(self, subset, inputs_path=None, return_labels = False):
        np.random.seed(self.random_seed)
        data_indices, newsgroups_data = self.get_data_indices(subset, inputs_path)
