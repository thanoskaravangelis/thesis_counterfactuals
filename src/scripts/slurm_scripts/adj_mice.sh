#!/bin/bash
#
#SBATCH --job-name=mice_adj # Όνομα για διαχωρισμό μεταξύ jobs
#SBATCH --nodes=1
...
cd /users/pa21/ptzouv/tkaravangelis/mice_attention
module purge
module load gnu/8 cuda/10.1.168 intelmpi/2018 pytorch/1.7.0
source /users/pa21/ptzouv/tkaravangelis/venv/bin/activate

start=$(date +%s.%N)
srun python /users/pa21/ptzouv/tkaravangelis/scripts/run_mice_adj_imdb.py
deactivate
