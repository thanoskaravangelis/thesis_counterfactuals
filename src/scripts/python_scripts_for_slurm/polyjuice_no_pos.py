...
sys.path.append('/users/pa21/ptzouv/tkaravangelis/mice_newsgroups/')
...
import re, time, logging, csv, signal
import torch
#local imports
...
def handler(signum, frame):
    raise Exception("Timeout of 2 minutes exceeded.")
...
for num_of_phase in tqdm(range (8, 11)):
    # ορίζω σαν νέο foldername την νέα φάση
    folder_name = f"polyjuice_newsgroups_random_{num_of_phase}"
    ...
    if num_of_phase != 0:
        ...
        prev_file = os.path.join(f"/users/pa21/ptzouv/tkaravangelis/polyjuice_results/polyjuice_newsgroups_random_{prev_phase}", "edits.csv")
